...
class avia_dynamic_template
{
	...
	*/
	...
	{
		$if_small_default_overwrite_with = "2D";
		$slider = new avia_slideshow($this->post_id, $if_small_default_overwrite_with);
	...
	*/
	...
	function hr($element)
	{
		...
		switch($element['saved_value'])
		{
			...
			case 'whitespace': 		$output .= "<div class='hr_invisible'></div>"; break;
		}
	...
	*/
	...
	function portfolio($element)
	{
		...
		$avia_config['portfolio_style'] = $portfolio_style;	
		...
		ob_start(); //start buffering the output instead of echoing it
		echo "<div class='template-portfolio-overview content portfolio-size-".$portfolio_columns." ".$portfolio_style."'>";
	...
	function post_page($element)
	{
		...
		if($additional_loop->have_posts())
		{
			...
			{ 
				...
				$output.= $content;
				if(avia_get_option('email_page') == $query_id) 
				{
					ob_start(); 
	...
	*/
	...
	function columns($element)
	{
		...
		$config_array  = array(
				...
				'1-4' => array( 'grid'=>'grid3' 	 , 'caption'=>false, 'image_size'=>'portfolio'),
		...
		for ($i = 1; $i <= $column_count; $i++)
		{
			...
			$output .= "<div class='".$grid.$first.$column_style."'>";
...
function avia_get_template()
{
	...
	*/
	...
	if(avia_get_option('frontpage') != "" && avia_get_option('blogpage') == $post->ID && !isset($avia_config['new_query']))
	{ 	
		...
		if(!empty($portfolios[0]['portfolio_page']) && !empty($portfolios[0]['portfolio_cats']))
		{
			...
			{			
				...
				{	
					...
					{		
					$avia_config['new_query'] = array("paged" => get_query_var( 'paged' ), "posts_per_page" => $portfolio['portfolio_item_count'],  'tax_query' => array( array( 'taxonomy' => 'portfolio_entries', 'field' => 'id', 'terms' => explode(',', $portfolio['portfolio_cats']) , 'operator' => 'IN')));
					}
