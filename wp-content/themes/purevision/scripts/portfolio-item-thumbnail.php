...
function get_portfolio_item_thumbnail( $num_cols='3', $width='260', $height='160', $do_not_link_adjacent_items = 'no', $echo = true ) {
    ...
    global $purevision_options, $post;
    ...
    if ( $portfolio_item_link )  {
        ...
        if( $portfolio_item_image_attachments == 'yes') {
            $rel_attr = ' rel="wp-prettyPhoto[portfolio-'.rand(1,1000).']"';
            $last_output = purevision_get_image_attachments( $post->ID, $rel_attr, $portfolio_item_offset_image_attachments, false );
        } else {
            $rel_attr = ( $do_not_link_adjacent_items == 'yes' ) ? ' rel="wp-prettyPhoto"' : ' rel="wp-prettyPhoto[portfolio]"';
        }
    ...
    if ( function_exists('get_the_image') ) { // the case when "Get The Image" plugin is available (installed and activated)
		...
		$output .= '<a'.$rel_attr.' href="'.$preview_item.'" title="'.$preview_item_title.'"'.$portfolio_item_link_target.'><img class="hover-opacity" src="'.purevision_process_image( $portfolio_thumb_as_array[url], $width, $height, 1, 90 ).'" width="'.$width.'" height="'.$height.'" alt="'.$portfolio_thumb_as_array[alt].'" /></a>';
	    } else { // if preview item is NOT available, generate a thumbnail that is NOT a link
		$output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col pngfix"> </span>';
		$output .= '<img class="hover-opacity" src="'.purevision_process_image( $portfolio_thumb_as_array[url], $width, $height, 1, 90 ).'" width="'.$width.'" height="'.$height.'" alt="'.esc_attr__('Preview item is not available!.', 'purevision').'" />';
	    }
		...
		if ( $preview_item ) { // if preview item is available, go ahead an link it to the thumbnail.
		    $output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col pngfix"> </span>';
		    $output .= '<a'.$rel_attr.' href="'.$preview_item.'" title="'.$preview_item_title.'"'.$portfolio_item_link_target.'><img class="hover-opacity" src="'.purevision_process_image( $portfolio_item_thumb, $width, $height, 1, 90 ).'" width="'.$width.'" height="'.$height.'" alt="'.get_the_title().'" /></a>';
		} else { // if preview item is NOT available, generate a thumbnail that is NOT a link
		    $output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col pngfix"> </span>';
		    $output .= '<img class="hover-opacity" src="'.purevision_process_image( $portfolio_item_thumb, $width, $height, 1, 90 ).'" width="'.$width.'" height="'.$height.'" alt="'.esc_attr__('Preview image not available!.', 'purevision').'" />';
		}
	    } elseif ( $preview_item ) { // auto generate thumbnails
		$output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col pngfix"> </span>';
		$output .= '<a'.$rel_attr.' href="'.$preview_item.'" title="'.$preview_item_title.'"'.$portfolio_item_link_target.'><img class="hover-opacity" src="'.purevision_process_image( $portfolio_item_preview, $width, $height, 1, 90 ).'" width="'.$width.'" height="'.$height.'" alt="'.get_the_title().'" /></a>';
	    } else { // Display default thumbnail image
		$output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col pngfix"> </span>';
		$output .= '<img class="hover-opacity" src="'.purevision_process_image( $portfolio_default_thumb, $width, $height, 1, 90 ).'" alt="'.esc_attr__("Default Image", 'purevision').'" width="'.$width.'" height="'.$height.'" />';
	    }
    }
    $output .= $last_output;
    if ( $echo )
 ...
 * @return string HTML string if $echo parameter is false.
 */
function purevision_get_image_attachments( $postID, $rel_attr, $offset = '0', $echo = true ) {
    $output = '';
    $args = array(
            'order'          => 'ASC',
            'orderby'        => 'menu_order',
